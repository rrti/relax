#ifndef RELAX_VEC3_HDR
#define RELAX_VEC3_HDR
...
template<typename T> struct vec3 {
	...
	inline T len2D() const { return T(std::sqrt(sqLen2D())); }
	inline T len3D() const { return T(std::sqrt(sqLen3D())); }
	...
	inline vec3<T>& randomize(INumberSequenceGen*);
